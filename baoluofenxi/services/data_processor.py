...
from models.models import ExperimentData, ExperimentType, EnvelopeSettings, EnvelopeCache, ExperimentType
...
class DataProcessor:
    ...
    def preview_file(self, file, experiment_type, preview_rows=10):
        ...
        try:
            ...
            if file.filename.endswith('.csv'):
                ...
                df = pd.read_excel(file, nrows=preview_rows)
    ...
    def process_upload(self, file, data_name, experiment_type):
        ...
        try:
            ...
            logging.info(f'数据上传成功: {data_name}, 行数: {len(df_clean)}, 表名: {clickhouse_result["table_name"]}')
    ...
    def upload_to_clickhouse(self, df, table_name, time_column, data_columns):
        ...
        try:
            ...
            # 创建表
            if not ch_manager.create_timeseries_table(table_name, time_column, data_columns):
                return {
                    'success': False,
                    'message': 'ClickHouse表创建失败'
                }
            ...
            return {
                'success': False,
                'message': f'ClickHouse上传失败: {str(e)}'
            }
    ...
    def validate_data_format(self, df, experiment_type):
        ...
        try:
            ...
            # 检查必要的列是否存在
            required_columns = [experiment_type.time_column] + experiment_type.data_columns
    ...
    def get_experiment_data(self, experiment_data_id, time_range=None, columns=None, limit=None):
        """从ClickHouse获取实验数据"""
        try:
            # 获取数据记录
    ...
    def get_multiple_experiment_data(self, experiment_data_ids, time_range=None, columns=None):
        """批量获取多个实验数据"""
        try:
            all_data = []
    ...
    def calculate_envelope_for_columns(self, experiment_type_id, selected_columns,
                                       sampling_points=None, use_sampling=True):
        """
        为指定列计算包络数据
        ...
        try:
            ...
            cache = EnvelopeCache.query.filter_by(
                experiment_type_id=experiment_type_id,
    ...
    def _compute_envelope_from_data(self, data_records, selected_columns):
        ...
        try:
            ...
            # 获取实验类型信息
            from models.models import ExperimentType
            experiment_type = ExperimentType.query.get(data_records[0].experiment_type_id)
            time_column = experiment_type.time_column
            ...
            # 按时间区间分组
            df['time_group'] = pd.cut(df[time_column], bins=time_bins, include_lowest=True)
            ...
            for column in selected_columns:
                ...
                upper_envelope = [np.sin(t * 2) + 1 + np.random.random() * 0.5 for t in time_points]
                lower_envelope = [np.sin(t * 2) - 1 - np.random.random() * 0.5 for t in time_points]
    ...
    def _compute_envelope_with_sampling(self, data_records, selected_columns, sampling_points):
        """
        计算采样包络 - 基于时间区间采样
    ...
    def _compute_envelope_full_data(self, data_records, selected_columns):
        ...
        try:
            ...
            for data_record in data_records:
                if not self.clickhouse_manager.table_exists(data_record.clickhouse_table_name):
                    continue
            ...
            for column in selected_columns:
                ...
                for time_point in sorted_time_points:
                    if column in all_data_by_time[time_point] and len(all_data_by_time[time_point][column]) > 0:
        ...
        try:
            stats = {
                ...
                'total_rows': db.session.query(
                    db.func.sum(ExperimentData.row_count)
                ).filter_by(
                    experiment_type_id=experiment_type_id,
                    status='active'
                ).scalar() or 0
    ...
    def process_temp_upload(self, file, experiment_type):
        """
        处理临时上传，用于包络分析对比
        ...
        try:
            ...
            if not clickhouse_result['success']:
                return {'success': False, 'message': f'ClickHouse上传失败: {clickhouse_result["message"]}'}
    ...
    def get_temp_comparison_data(self, temp_table_name, time_column, selected_columns,
                                 use_sampling=True, sampling_points=200):
        """
        从临时表获取对比数据
        ...
        try:
            # 构建查询语句
            columns_str = f"`{time_column}`, " + ', '.join([f'`{col}`' for col in selected_columns])
            query = f"""
            SELECT {columns_str}
            ...
            for column in selected_columns:
                if column not in data[0]:
                    logging.error(f"列 '{column}' 不在数据中，可用列: {list(data[0].keys())}")
    ...
    def save_temp_data_to_mysql(self, temp_table_name, data_name, experiment_type, file_name):
        """
        将临时数据保存到MySQL元数据表
        """
        try:
            ...
            experiment_data = ExperimentData(
                ...
                status='active',
                row_count=row_count,
                clickhouse_table_name=temp_table_name.replace('temp_', ''),  # 移除temp前缀
                upload_time=datetime.now()
            )
            ...
            if not rename_result['success']:
                ...
                db.session.delete(experiment_data)
                db.session.commit()
                return {'success': False, 'message': f'表重命名失败: {rename_result["message"]}'}
    ...
    def calculate_envelope_simple(self, experiment_type_id, selected_columns):
        ...
        try:
            ...
            for data_record in historical_data:
                ...
                # 检查表是否存在
                if not self.clickhouse_manager.table_exists(data_record.clickhouse_table_name):
                    logging.warning(f"ClickHouse表 {data_record.clickhouse_table_name} 不存在")
                    continue
                ...
                result = self.clickhouse_manager.execute_query(query)
                if not result['success']:
                    logging.error(f"查询表 {data_record.clickhouse_table_name} 失败: {result['message']}")
                    continue
                
                # 调试：打印查询结果的键
                if result['data']:
                    actual_columns = list(result['data'][0].keys())
                    logging.info(f"表 {data_record.clickhouse_table_name} 查询结果字段: {actual_columns}")
                
                # 处理数据
                for row in result['data']:
                    # 检查时间列是否存在
                    if time_column not in row:
                        logging.error(f"时间列 '{time_column}' 不在查询结果中: {list(row.keys())}")
                        continue
                    ...
                    for column in selected_columns:
                        if column in row:
                            ...
                            logging.warning(f"请求的列 '{column}' 不存在，可用列: {actual_columns}")
            ...
            for column in selected_columns:
                ...
                for time_point in sorted_time_points:
                    if column in all_envelope_data and time_point in all_envelope_data[column]:
